...
const Contact = () => {
  ...
  return (
    <Layout>
     <div page={'contact'} className={`px-12 pb-72 text-gray-700 min-h-screen flex justify-center bg-gray-100 transition-all ${showMenu ? "blur-sm duration-500" : "blur-none duration-200"}`}>

      {!emailSent &&
        <div className={`w-full flex flex-col max-w-5xl`}>

          <div className={`text-xl py-8 text-gray-600 font-serif`}>Send the elders an email</div>

          <input className={`w-full bg-white border px-2 py-2`} placeholder={'Subject...'}
          value={emailSubjectText} onChange={(e) => setEmailSubjectText(e.target.value)}></input>

          <br />

          <textarea className={`w-full min-h-[300px] max-h-[500px] bg-white p-2`} placeholder={'Write us an email here...'}
