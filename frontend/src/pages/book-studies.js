      ...
      <div className={`flex flex-col items-center justify-between`}>
        ...
        <div className={`flex flex-col items-center`}>
          ...
          <a href={node.linkToPurchase} target="_blank">
            <GatsbyImage className={``} image={node.cover.asset['gatsbyImageData']} alt={node.title} />    
          </a>        
          
        </div>
      
        <div className={`flex flex-col space-y-2 pt-8 lg:pt-0 lg:pb-8 items-center`}>
          <div className={`text-lg lg:text-xl text-gray-600`}>{d > Date.now() ? "Starting" : "Started"} {d.toLocaleDateString()}</div>
          <div className={`text-lg lg:text-xl text-gray-600`}>{day}s, {d.toLocaleTimeString([], { timeStyle: 'short', })}</div>      
    ...
    <Layout>
      <div page={'book-studies'} className={`pt-20 flex justify-center bg-gray-100`}>        
        ...
        <div className={`max-w-[1200px] pb-96`}>
          ...
          <div className={`px-8 py-8 min-h-screen bg-gray-100 transition-all 
