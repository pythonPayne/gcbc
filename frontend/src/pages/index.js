...
const Home = ({ data }) => {
  ...
  useEffect(() => {
    ...
    setCarouselImages(data.allSanityPictures.edges.map((edge,i) => {
      return {
        startPosition: i+1,
        position: i+1,
  ...
  const carouselImage = (position) => {
    ...
    if (image) {  
    return (
    <div className={`w-full shrink-0 h-full transition-all ease-in-out
  ...
  return (
    <Layout>
      <div page={'home'} className={`pb-48 min-h-screen bg-gray-100 flex flex-col items-center pt-12 justify-center transition-all ${showMenu ? "blur-sm duration-500" : "blur-none duration-200"}`}> 
