...
const Confession = ({data}) => {  
  ...
  const numberOfChapters = Math.max(...confession.map(edge => edge.node.chapter)) 
  ...
  const confession_chapter_card = (chapter) => {
    ...
    const numberOfParagraphs = Math.max(...chapter.map(edge => edge.node.paragraph))
    const paragraphNums = Array(numberOfParagraphs).fill(0).map((e,i)=>i+1)
