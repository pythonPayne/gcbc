...
const About = ({data}) => {  
  ...
  const gcbcCard = (title, order1, order2) => {
    ...
    const words = <div className={`flex flex-col ${order2} md:order-none pb-28 md:pb-0`}>
                    <div className={`text-4xl font-bold py-6 text-gray-600`}>{edge.node.title}</div>
    ...
    return (
      <>
        ...
        <div className={`flex flex-col pb-32 md:pb-0`}>
          <div className={`text-4xl font-bold py-6 text-gray-600`}>{elder.node.title}</div>
    ...
    <div page={'about'} className={`pt-20 flex justify-center bg-gray-100`}>        
      ...
      <div className={`max-w-[1200px]`}>
        ...
        <div className={`px-12 pt-12 pb-36 min-h-screen flex flex-col justify-center bg-gray-100 transition-all ${showMenu ? "blur-sm duration-500" : "blur-none duration-200"}`}>
          ...
          tracking-widest border-b-2`}>G &nbsp;C &nbsp;B &nbsp;C</div>
          ...
          {/* OUR ELDERS */}
          <div className={`py-4 mb-12 md:mb-16 flex justify-center text-[#09314C] text-5xl md:text-6xl font-sans font-bold 
          tracking-widest border-b-2`}>Our Elders</div>
