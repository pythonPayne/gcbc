...
const Layout = (props) => {
  ...
  const navigation_button_links = (links) => {
    const linkClass = "text-sm whitespace-nowrap px-2 py-2 text-gray-600 hover:text-blue-700"
    return (
      <div className={`shadow-2xl absolute ring-1 ring-gray-200 -mt-[.04rem] z-10 bg-white pl-2 pr-24 pb-2 flex flex-col pt-2`}>
        {links.map((link) => {
          if (link.tag === "Link")
            return (
              <Link key={link.route} href={link.route} className={linkClass}>
                {link.text}
              </Link>
            )
          else if (link.tag === "a")
            return (
              <a key={link.route} href={link.route} target="_blank" className={linkClass}>
                {link.text}
              </a>
            )
        })}
  ...
  const side_menu_link = (link) => {
    ...
    if (link.tag === "Link")
      return (
        <Link key={link.route} className={linkClass} href={link.route}>
          {link.text}
        </Link>
      )
    else if (link.tag === "a")
      return (
        <a key={link.route} className={linkClass} href={link.route} target={"_blank"}>
          {link.text}
        </a>
      )
  }
  ...
  return (
    <>
        ...
        <div className={`flex h-full justify-between items-center pl-1 pr-4 md:pl-5 md:pr-10`}>
          ...
          >
            <div className={`transition-all duration-200 bg-gray-400
              ${showMenu ? "w-[.15rem] h-5 rotate-45 translate-y-[.715rem]" : "h-[.15rem] w-5"}`}
            ></div>
            <div className={`transition-all duration-200 bg-gray-400
              ${showMenu ? "w-[.15rem] h-5 -rotate-45 -translate-y-[.715rem]" : "h-[.15rem] w-5"}`}
            ></div>
          ...
          <div className={`hidden lg:flex lg:space-x-5 lg:font-sans lg:items-center`}>
            {navigation_button("About", [
              { tag: "Link", route: "/about", text: "About Us" },
              { tag: "Link", route: "/beliefs", text: "Beliefs" },
              { tag: "Link", route: "/church-covenant", text: "Church Covenant" },
              { tag: "a", route: "https://drive.google.com/drive/folders/1My0xUd9s0d3Rn2bGjeoLn_YeONPIpzpD?usp=share_link", text: "Members" },
            ])}
            {navigation_button("Media", [
              { tag: "Link", route: "/sermons", text: "Sermons" },
              { tag: "a", route: "https://www.sermonaudio.com/solo/gracecovenantbaptist/webcast/", text: "Livestream" },
            ])}
            {navigation_button("Studies", [
              { tag: "Link", route: "/book-studies", text: "Book Studies" },
            ])}
            {navigation_button("Outreach", [
              { tag: "Link", route: "/outreach", text: "Ministries" },
            ])}
            {navigation_button("Calendar", [
              { tag: "Link", route: "/calendar", text: "Calendar" },
            ])}
          </div>
        </div>
      </div>
          ...
          <div className={`relative`}>
            <div className={`absolute pb-[50vh] w-full pl-4 pr-4 pt-8`}>
              <div className={`flex flex-col space-y-2`}>
                {side_menu_link({ tag: "Link", route: "/about", text: "About Us" })}
                {side_menu_link({ tag: "Link", route: "/beliefs", text: "Beliefs" })}
                {side_menu_link({ tag: "Link", route: "/church-covenant", text: "Church Covenant" })}
                {side_menu_link({ tag: "a", route: "https://drive.google.com/drive/folders/1My0xUd9s0d3Rn2bGjeoLn_YeONPIpzpD?usp=share_link", text: "Members" })}
                <hr className="border border-gray-200" />
                {side_menu_link({ tag: "Link", route: "/sermons", text: "Sermons" })}
                {side_menu_link({ tag: "a", route: "https://www.sermonaudio.com/solo/gracecovenantbaptist/webcast/", text: "Livestream" })}
                <hr className="border border-gray-200" />
                {side_menu_link({ tag: "Link", route: "/book-studies", text: "Book Studies" })}
                {side_menu_link({ tag: "Link", route: "/outreach", text: "Outreach" })}
                {side_menu_link({ tag: "Link", route: "/calendar", text: "Calendar" })}
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Footer */}
      <div>
        <div className={`bg-[#09314C] text-gray-300 text-opacity-80 grid grid-cols-1 text-center gap-y-[3rem] py-28 lg:grid-cols-2`}>
          <div className={`py-10 tracking-wide text-lg lg:text-xl`}>
            ...
            <div className={`pt-2 space-y-1`}>
              <div><a href={`tel: 205-426-2234`}>205-426-2234</a></div>
              <div><a href={`mailto: pastor@gracecovenantbaptist.org`}>pastor@gracecovenantbaptist.org</a></div>
            </div>
            <div className={`pt-3`}>
              <a className={`underline`} target="_blank" href="https://www.google.com/maps/dir//Grace+Covenant+Baptist+Church,+2565+Rocky+Ridge+Rd,+Vestavia+Hills,+AL+35243/@33.4295652,-86.8408153,12z/data=!4m8!4m7!1m0!1m5!1m1!1s0x888918455c0624b3:0x25d19d79c0c6e263!2m2!1d-86.7707752!2d33.4295849">
                2565 Rocky Ridge Road<br />Vestavia Hills Alabama 35243
              </a>
            </div>
          </div>
          <div className={`py-10 tracking-wide text-lg lg:text-xl`}>
        ...
        <div className={`bg-[#09314C] text-gray-500 flex justify-center text-center py-12 italic`}>
          <div>
            <div>Images from: <a className="cursor-pointer" href="https://unsplash.com" target="_blank">Unsplash</a></div>
            <div>Powered by: <a className="cursor-pointer" href="https://netlify.com" target="_blank">Netlify</a></div>
          </div>
        </div>
      </div>
    </>
