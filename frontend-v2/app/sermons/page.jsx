...
async function getSermons() {
  return await client.fetch(`*[_type == "sermons"] {
    _id, date, speaker, title, passage, book, audioLink
  }`)
}
...
export default function Sermons() {
  ...
  const booksOT = ["Genesis", "Ruth", "Esther", "Psalms", "Ecclesiastes", "Isaiah", "Jonah", "Habakkuk"]
  const booksNT = ["Matthew", "Mark", "Luke", "John", "Acts", "Romans", "1 Corinthians", "2 Corinthians",
    "Galatians", "Ephesians", "Philippians", "Colossians", "1 Timothy", "2 Timothy", "Philemon",
    "Hebrews", "1 Peter", "2 Peter", "1 John", "2 John", "3 John", "Jude", "Revelation"]
  const speakers = ["Todd Wilson", "Stephen Hyde", "Stephen Payne", "Steve Cowan"]

  useEffect(() => {
    dispatch(toggleShowMenu(false))
    getSermons().then((data) => {
      setSermons(data.map((s) => ({ ...s, bookPassage: s.book + " " + s.passage })))
    })
  }, [])

  useEffect(() => {
    setTimeout(() => setSorting(false), 1500)
  }, [sortDir])

  const sermon_card = (sermon) => {
    return (
      <div key={sermon._id} className={`bg-white relative grid place-content-center text-center h-52 md:h-56 shadow-md`}>
        <div className={`flex flex-col px-4`}>
          <a className={`font-serif text-xl font-semibold text-[#09314C]/80 hover:text-[#09314C]/60`}
            href={sermon.audioLink} target="_blank">
            {sermon.title}
          </a>
        ...
        <div className={`absolute bottom-0 flex w-full justify-between items-center py-2 px-4`}>
          <div className={`text-left`}>
            ...
            <div className={`text-xs transition-colors ${sorting ? "bg-[#09314C] text-white" : "bg-white text-gray-400"}`}>
              {sermon.date}
            </div>
          </div>
          <a className={`text-md text-gray-400`} href={sermon.audioLink} target="_blank">
  ...
  let filteredSermons = sermonBookFilter ? sermons.filter((s) => s.book === sermonBookFilter) : sermons
  filteredSermons = sermonSpeakerFilter ? filteredSermons.filter((s) => s.speaker === sermonSpeakerFilter) : filteredSermons
  filteredSermons = sermonSearchFilter
    ? filteredSermons.filter((s) =>
        s.title?.toLowerCase().includes(sermonSearchFilter.toLowerCase()) |
        s.book?.toLowerCase().includes(sermonSearchFilter.toLowerCase()) |
        s.bookPassage?.toLowerCase().includes(sermonSearchFilter.toLowerCase()) |
        s.date?.toLowerCase().includes(sermonSearchFilter.toLowerCase()) |
        s.speaker?.toLowerCase().includes(sermonSearchFilter.toLowerCase())
      )
    : filteredSermons
    ...
    <Layout>
      <div className={`${showMenu && "pointer-events-none"} pt-20 flex justify-center bg-gray-100`}>
        <div className={`min-h-screen max-w-[1200px] w-[100vw] md:w-[70vw] pb-96`}>
          ...
          {showSermonsFilterMenu && (
            <div className={`flex flex-col`}>
              <div className={`flex justify-between mx-8 py-8`}>
                <button
                  className={`text-xs bg-red-600/80 ring-1 ring-red-600 py-3 px-4 text-white shadow-md
                    ${sermonBookFilter === null && sermonSpeakerFilter === null && "invisible"}`}
                  onClick={() => {
                ...
                <button
                  ...
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} className="w-6 h-6 stroke-gray-800">
              ...
              <div className={`flex flex-col mx-8 border shadow-md mb-4`}>
                <button
                  className={`flex justify-between px-8 py-4 bg-[#09314C]/50 text-white ring-1 ring-[#09314C]`}
                ...
                <div className={`${filterBookOpen ? "h-full p-4 overflow-auto text-gray-900" : "h-0 p-0 overflow-hidden text-white"}`}>
                  <div className={`grid grid-cols-2 gap-x-4 gap-y-2 mb-4`}>
                    ...
                    {booksOT.map((book, i) => (
                      <div key={i}
                        className={`text-center text-sm p-2 border border-gray-300 cursor-pointer ${sermonBookFilter === book && "bg-gray-300"}`}
                        onClick={() => dispatch(setSermonBookFilter(book !== sermonBookFilter ? book : null))}>
                        {book}
                      </div>
                    ))}
                  </div>
                ...
                <div className={`flex justify-center ${filterSpeakerOpen ? "h-full p-4 overflow-auto text-gray-900" : "h-0 p-0 overflow-hidden text-white"}`}>
                  <div className={`flex flex-col justify-center w-[50%] space-y-5`}>
                    {speakers.map((speaker, i) => (
                      <div key={i}
                        className={`text-sm text-center px-4 py-2 border border-gray-300 cursor-pointer ${sermonSpeakerFilter === speaker && "bg-gray-300"}`}
                        onClick={() => dispatch(setSermonSpeakerFilter(speaker !== sermonSpeakerFilter ? speaker : null))}>
                        {speaker}
                      </div>
                    ))}
                  </div>
          ...
          {!showSermonsFilterMenu && (
            <div className={`pb-36 px-6 transition-all ${showMenu ? "blur-sm duration-500" : "blur-none duration-[200]"}`}>
              <div className={`py-8`}>
                <div className={`shadow-md rounded-md mb-2 text-gray-800`}>
                  <input
                    className={`bg-gray-200 py-2 w-full px-2 text-xl placeholder:text-gray-400`}
                    type="text"
              ...
              <div className={`grid grid-cols-1 gap-6 md:gap-10 md:grid-cols-2 2xl:grid-cols-3`}>
                {filteredSermons.slice(0, 50).map((sermon) => sermon_card(sermon))}
              </div>
            </div>
          )}
        </div>
